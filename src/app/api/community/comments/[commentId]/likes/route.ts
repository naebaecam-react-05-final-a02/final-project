...
export async function PATCH(request: NextRequest, { params }: { params: { commentId: string } }) {
  ...
  try {
    ...
    // 4. communityComment 테이블 업데이트
    const { error: updateError } = await supabase.from('communityComment').update({ likes: count }).eq('id', commentId);
...
export async function GET(request: NextRequest, { params }: { params: { commentId: string } }) {
  ...
  try {
    const { data, error } = await supabase.from('communityCommentLikes').select('userId').eq('commentId', commentId);
