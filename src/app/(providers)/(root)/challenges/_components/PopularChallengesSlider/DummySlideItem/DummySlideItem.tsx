...
const DummySlideItem = ({ challenge, index, activeIndex }: SlideItemProps) => {
  const pos =
    index === activeIndex
      ? 'active'
      : index === activeIndex - 1
    ...
    <div className={SlideItemVariants({ pos })}>
      <div className="flex flex-col gap-3">
        <div className="flex gap-2">
          ...
          <h3 className="text-xl font-semibold">더미입니다</h3>
        </div>
        <div className="flex text-sm gap-3 bg-white/10 px-[8px] py-[3px] rounded-[4px]">
          <p className="text-primary-100">참여 100</p>
          <p>인증 100</p>
        </div>
      </div>
      <div className="w-full flex flex-col gap-2">
        <div className="flex">
          <Bullet />
          <p className="text-[16px] font-semibold">더미입니다</p>
        </div>
